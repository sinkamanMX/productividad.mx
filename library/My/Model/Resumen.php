...
{	
	...
	public function getCitasPendientes(){
		$result= Array();
		$this->query("SET NAMES utf8",false); 		
    	$sql ="SELECT  C.ID_CITA, C.FECHA_CITA, IF(C.HORA_CITA>'24:00:00','00:00:00',C.HORA_CITA) AS HORA_CITA , C.FOLIO, E.DESCRIPCION AS N_ESTATUS, E.ID_ESTATUS, 
    					T.DESCRIPCION AS N_TIPO, R.ID_USUARIO, CONCAT(C.FECHA_CITA,' ',IF(C.HORA_CITA>'24:00:00','00:00:00',C.HORA_CITA)) AS FECHA_INICIO, 		
						IF(C.FECHA_TERMINO IS NULL, DATE_SUB(CONVERT(CONCAT(C.FECHA_CITA,' ',IF(C.HORA_CITA>'24:00:00','00:00:00',C.HORA_CITA)), DATETIME), INTERVAL -1 HOUR) ,C.FECHA_TERMINO) AS FECHA_FIN,
						L.RAZON_SOCIAL
    	/*SELECT  C.ID_CITA, C.FECHA_CITA, C.HORA_CITA, C.FOLIO, E.DESCRIPCION AS N_ESTATUS, E.ID_ESTATUS, 
	    			...
	    			ORDER BY C.FECHA_CITA , C.HORA_CITA ASC
	    			LIMIT 50";    	    	
		$query   = $this->query($sql);
		if(count($query)>0){		  
			$result = $query;			
