...
class My_Model_Horarios extends My_Db_Table
{
	...
	public function getCbo($aSucursales,$fecha){
		$result= Array();
    	...
    	$sql ="SELECT ID_HORARIO AS ID, CONCAT(HORA,'-',HORA_FIN) AS NAME
				...
				( 
					...
					WHERE H.ID_SUCURSAL IN ($aSucursales)
					  AND A.DIA = '$fecha')";
    ...
    public function getUserAssign($aSucursales,$fecha){
		$result= -1;
		$this->query("SET NAMES utf8",false); 
    	$sql =" SELECT ID_USUARIO
				 ...
				 (
					...
					INNER JOIN PROD_HORARIO_ASIGNADO A ON H.ID_HORARIO = A.ID_HORARIO
					WHERE A.DIA = '$fecha'
					  AND H.ID_SUCURSAL IN ($aSucursales)
				 )
